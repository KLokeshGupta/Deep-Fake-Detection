"""
CNN is based on vgg16; look at base import section
...
class BlinkCNN(object):
    ...
    def __init__(self,
        ...
        with open(cfg_file, 'r') as f:
            # cfg = edict(yaml.load(f,Loader=FullLoader))
...
class BlinkLRCN(object):
    ...
    #     input_shape = (self.batch_size, self.max_time, *self.img_size)
    #     blined_gt_shape = (self.batch_size,)
    #     eye_state_gt_shape = (self.batch_size, self.max_time)
    #     seq_len_shape = (self.batch_size,)

    #     # Initialize inputs as instance variables
    #     self.input = tf.zeros(input_shape, dtype=tf.float32)
    #     self.blined_gt = tf.zeros(blined_gt_shape, dtype=tf.int32)
    #     self.eye_state_gt = tf.zeros(eye_state_gt_shape, dtype=tf.int32)
    #     self.seq_len = tf.zeros(seq_len_shape, dtype=tf.int32)
    ...
    def _vgg16(self, input):
        ...
        # return cnn_out
        with tf.compat.v1.variable_scope('vgg16_scope', reuse=tf.compat.v1.AUTO_REUSE):
        # Reshape from NxTxHxWxC to (NxT)xHxWxC
            input = tf.reshape(input, [-1, self.img_size[0], self.img_size[1], self.img_size[2]])
            layers = base.get_vgg16_pool5(input, self.params)
            layers.fc6 = net_ops.fully_connected(input=layers.pool5, num_neuron=4096, name='fc6', params=self.params)
    ...
    def _rnn_cell(self, input):
        with tf.compat.v1.variable_scope('rnn_cell', reuse=tf.compat.v1.AUTO_REUSE):
