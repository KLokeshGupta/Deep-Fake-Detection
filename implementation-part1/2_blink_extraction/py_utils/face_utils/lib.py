...
def draw_convex_hull(im, points, color):
    ...
    cv2.fillPoly(im, [points], color=color)
...
def get_face_loc(im, face_detector):
    ...
    face_locs = []
    faces = list(face_detector(np.uint8(im)))
...
def get_aligned_face_and_landmarks(im, face_cache, aligned_face_size = 256, padding=(0, 0)):
    ...
    for mat, points in face_cache:
        ...
        aligned_cur_shapes.append(aligned_pred) #point of aligned facial landmark
        aligned_cur_im.append(aligned_face) #image aligned
...
def crop_eye(img, points):
    ...
    for j in range(len(eyes)): #do on each eye
        lp = np.min(eyes[j][:, 0]) #eye coordinate on x-axis
        rp = np.max(eyes[j][:, 0])
        tp = np.min(eyes[j][:, -1])#eye coordinate on y-axis
        bp = np.max(eyes[j][:, -1])

        w = rp - lp
        h = bp - tp
        # update coordinate of the eye with offset
    ...
    return eyes_list #return eyes images
