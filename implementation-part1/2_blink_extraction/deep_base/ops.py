...
def max_pool(input, name, ksize=(2, 2), strides=(2, 2), padding='SAME'):
    with tf.name_scope(name) as scope:
        out = tf.keras.layers.MaxPooling2D(pool_size=ksize, strides=strides, padding=padding)(input)
        print('{} max pool out: {}'.format(name, out))
        return out
...
def avg_pool(input, name, ksize=(2, 2), strides=(2, 2), padding='SAME'):
    with tf.name_scope(name) as scope:
        out = tf.keras.layers.AveragePooling2D(pool_size=ksize, strides=strides, padding=padding)(input)
        print('{} avg pool out: {}'.format(name, out))
        return out
...
def batch_norm(input, name, is_train=True, params={}):
    batch_norm_out = tf.keras.layers.BatchNormalization(name=name)(input, training=is_train)
    var_list = tf.compat.v1.get_collection(tf.compat.v1.GraphKeys.GLOBAL_VARIABLES, scope=name)
    params[name] = var_list
    print('{} {}'.format(name, batch_norm_out))
    return batch_norm_out
...
def get_restore_var_list(path):
    ...
    saved_vars_name = [var[0] for var in saved_vars]
    restore_var_list = [var for var in global_vars if var.name[:-2] in saved_vars_name]
    return restore_var_list
