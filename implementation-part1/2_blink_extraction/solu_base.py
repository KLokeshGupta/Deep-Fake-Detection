...
This code is used for:
    ...
    2. Cropping frames for getting the eye area
    3. Append the eye state
    4. Generating the eye state graphs
    5. Generate the scaled concatenate diagram (frame+eyestategraph) into video.
"""
...
class Solu(object):
    ...
    def __init__(self,
        ...
        for i, im in enumerate(tqdm(self.imgs)):
            ...
            # crop eyes
            leye, reye = lib.crop_eye(aligned_img[0], aligned_shapes_cur[0]) #for first face. No prob as we filtered all the frame for only 1 face
            self.left_eyes.append(leye) #store the left eye np value of im
            self.right_eyes.append(reye) #store the right eye np value of im
    ...
    def gen_videos(self, out_dir, tag=''):
        ...
        pv.gen_vid(str(out_path), final_list, self.fps)
